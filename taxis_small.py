...
taxis = pd.read_csv("taxis_small.csv")
...
knnResults = cross_validate(knnModel,X_train, y_train,cv=kf, scoring= "r2", return_train_score=True)
...
SLRModelResults = cross_validate(SLRModel, X_train, y_train, cv=kf, scoring="r2", return_train_score=True)
